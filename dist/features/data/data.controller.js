...
const fetchPersons = async (req, res) => {
    try {
        const persons = await (0, data_service_1.getPersonsToMeet)();
...
const createPersonHandler = async (req, res) => {
    try {
        const person = await (0, data_service_1.createPerson)(req.body);
...
const updatePersonHandler = async (req, res) => {
    try {
        ...
        const person = await (0, data_service_1.updatePerson)(id, req.body);
...
const deletePersonHandler = async (req, res) => {
    try {
        ...
        if (isNaN(id))
            return res.status(400).json({ error: 'Invalid ID' });
        await (0, data_service_1.deletePerson)(id);
